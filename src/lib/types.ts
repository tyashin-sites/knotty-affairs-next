...
export interface ApiProduct {
  ...
  viewCount: number;
  soldCount: number;
  /** Denormalized from published verified-buyer reviews (0/absent = none). */
...
/** Lightweight item shape of GET /products/:slug/related — NOT an ApiProduct. */
...
/** One reviewable purchase from GET /reviews/eligible (session-identified). */
...
export interface ProductReviewsPayload {
  /** false ⇒ the store hasn't enabled review display — render nothing. */
  enabled?: boolean;
...
export interface StoreInfo {
  ...
  /** Structured return/replacement/refund facts; the document is /return-policy. */
  returnPolicy?: {
    category: 'not-permitted' | 'finite';
    merchantReturnDays?: number;
    returnFees?: 'free' | 'customer-paid';
    defectiveItemWindowDays?: number;
...
export interface StoreShippingZone {
  ...
  freeAbove?: number; // smallest currency unit
  estimatedDays?: string; // free-form, e.g. "3-5"
}
