...
export const api = {
  ...
  // NOTE: unlike /products, this endpoint wraps the list — data is
  // { products: [...] } of LIGHTWEIGHT RelatedProduct items (id/thumbnailUrl,
  // no images array). Typing this as ApiProduct[] took every PDP down on
  // 2026-08-03 (.slice, then .images.find, on the wrong shape).
  getRelatedProducts: (slug: string, limit = 8) =>
    apiFetch<{ products: RelatedProduct[] }>(
      `/products/${encodeURIComponent(slug)}/related?limit=${limit}`,
      { noSession: true, revalidate: 300 },
    ),

  getProductReviews: (productId: string, page = 1, limit = 10) =>
    apiFetch<ProductReviewsPayload>(
      `/products/${encodeURIComponent(productId)}/reviews?page=${page}&limit=${limit}`,
      { noSession: true, revalidate: 60 },
    ),

  /* -- identified inline reviews (browser-only: needs the session id) ------ */

  getEligibleReviews: () =>
    apiFetch<{ enabled: boolean; items: EligibleReviewItem[] }>('/reviews/eligible', {
      noStore: true,
    }),
  ...
  submitReview: (
    ...
    apiFetch<{ id: string; status: string; message: string }>(
      ...
      { method: 'POST', body: JSON.stringify(data), noStore: true },
    ),
